...
class Menu:
			...
			if self.opciones[self.opcion] == idioma.lang.play:
				if self.select_car() == 0:
					if self.select_circuit():
						return 1
			if self.opciones[self.opcion] == "Multiplayer (under development)":
				self.macli = self.select_macli()
				if self.macli == "cliente":
					self.macli_menu()
				elif self.macli == "servidor":
					self.master_menu()
				else:
					self.client_menu()
	...
	def text(self):
		...
		self.opciones = (idioma.lang.play, "Multiplayer (under development)",idioma.lang.options, idioma.lang.hof, idioma.lang.credits, idioma.lang.quit)
 ...
 Se devuelven el objeto car, el color del coche, el circuito cargado y el crono"""
		...
		text = font.render(idioma.lang.loading, 1, (10, 10, 10))
		i,j = font.size(idioma.lang.loading)
		...
		return car, self.car_color, circuit, crono
...
#(160, 90, 190, 255), (230, 230, 230, 255), (255, 150, 0, 255), (67, 171, 0, 255), (14, 150,0, 255)]
	...
	def select_macli(self):
		...
		select = self.menu_opt(opciones, "selecciona master o cliente")
		return select
	...
	def master_menu(self):
		...
		self.n = int(self.menu_opt(range(1,10), "Numero de jugadores"))
		self.nick = self.recibir_teclado("Introduce el nick:")
	...
	def macli_menu(self):
		...
		self.s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		if addr == 0:
			servidor = self.recibir_teclado("Introduce la IP del servidor:")
	...
	def master_menu2(self):
		...
		self.n = int(self.recibir_teclado2("Numero de jugadores"))
		self.nick = self.recibir_teclado("Introduce el nick:")
		self.nick = self.nick+";"+str(self.select)
